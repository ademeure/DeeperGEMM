...
CUtensorMap make_2d_tma_copy_desc(T* global_address, uint64_t gmem_dim[2],
    ...
    auto result = encode_func(
            ...
            // No longer need 256B promotion for B since we load consecutive 4096B
            // TODO - do we want to add this back only for A & other tensors?
...
CUtensorMap make_3d_tma_copy_desc(T* global_address, uint64_t gmem_dim[3],
    ...
    uint64_t global_stride[rank - 1] = {stride_in_bytes[0], stride_in_bytes[1]};
...
template <bool kTMAMulticastEnabled = false>
__device__ __forceinline__ void
tma_copy(void const* desc_ptr, uint64_t* barrier_ptr, void* smem_ptr,
         int32_t const& crd_0, int32_t const& crd_1, uint32_t num_multicast = 1) {
    ...
    } else { // need to check for rank_in_cluster() == 0 outside the function(/loop)
        cute::SM90_TMA_LOAD_MULTICAST_2D::copy(desc_ptr, barrier_ptr, (1 << num_multicast) - 1, cache_hint, smem_ptr, crd_0, crd_1);
    }
}
